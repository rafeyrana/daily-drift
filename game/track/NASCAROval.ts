...
// Use examples/jsm path to match installed three version and available exports
...
export function createNASCAROval(
  ...
  const wallsGroup = options.enableWalls
    ? createOuterWalls(centerlineSamples, options, materials.wall)
    : null;
  const rumbleGroup = createRumbleStrips(centerlineSamples, options, materials.rumble);
  const fenceGroup = options.enableWalls
    ? createSafetyFence(centerlineSamples, options, materials.fence)
    : null;
...
function generateNASCAROvalCenterline(options: GameOptions): CenterlineSample[] {
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    if (distanceAlongTrack < straightSegmentLength) {
      ...
      position = new THREE.Vector3(-straightLength / 2 + progress * straightLength, 0, 0);
      ...
      // Back straight
      const progress =
        (distanceAlongTrack - straightSegmentLength - cornerArcLength) / straightSegmentLength;
      position = new THREE.Vector3(
      ...
      const progress =
        (distanceAlongTrack - 2 * straightSegmentLength - cornerArcLength) / cornerArcLength;
...
function createBankedTrackSurface(
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    const bankedUp = surfaceUp
      .clone()
      .applyAxisAngle(sample.tangent, Math.atan2(sample.normal.y, sample.normal.z));
...
function createPaintedLines(
  ...
  const innerLine = createLineStrip(
    centerlineSamples,
    innerLineOffset,
    lineWidth,
    lineHeight,
  ...
  const outerLine = createLineStrip(
    centerlineSamples,
    outerLineOffset,
    lineWidth,
    lineHeight,
...
function createOuterWalls(
  ...
  for (let i = 0; i < centerlineSamples.length; i += 8) {
    // Every 8th sample
    const sample = centerlineSamples[i];
  ...
  // Create merged wall geometry using BufferGeometryUtils
  ...
  wallGeometries.forEach((geometry, index) => {
    ...
    transformedGeometries.push(cloned);
  });

  // Older three exports `mergeGeometries` instead of `mergeBufferGeometries`
  const mergedGeometry = (BufferGeometryUtils as any).mergeBufferGeometries
    ? (BufferGeometryUtils as any).mergeBufferGeometries(transformedGeometries, false)
    : (BufferGeometryUtils as any).mergeGeometries(transformedGeometries, false);
...
function createRumbleStrips(
  ...
  for (let i = 0; i < centerlineSamples.length; i += 20) {
    ...
    // Inner rumble
    const innerRumble = createRumbleSegment(
      startSample,
      endSample,
      -options.trackWidth / 2 + rumbleWidth / 2,
      rumbleWidth,
      rumbleHeight,
      material
    );
    ...
    // Outer rumble
    const outerRumble = createRumbleSegment(
      startSample,
      endSample,
      options.trackWidth / 2 - rumbleWidth / 2,
      rumbleWidth,
...
function createSafetyFence(
  ...
  const postMesh = new THREE.InstancedMesh(
    postGeometry,
    postMaterial,
    Math.floor(centerlineSamples.length / 10)
  );
